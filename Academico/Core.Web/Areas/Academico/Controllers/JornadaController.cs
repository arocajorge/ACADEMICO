...
namespace Core.Web.Areas.Academico.Controllers
{
    public class JornadaController : Controller
    {
        ...
        public ActionResult Nuevo(aca_Jornada_Info model)
        {
            ...
            return RedirectToAction("Consultar", new { IdEmpresa = model.IdEmpresa, IdJornada = model.IdJornada, Exito = true });
        }

        public ActionResult Consultar(int IdEmpresa = 0, int IdJornada = 0, bool Exito = false)
        {
            #region Validar Session
            if (string.IsNullOrEmpty(SessionFixed.IdTransaccionSession))
                return RedirectToAction("Login", new { Area = "", Controller = "Account" });
