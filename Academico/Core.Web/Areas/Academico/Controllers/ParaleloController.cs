...
namespace Core.Web.Areas.Academico.Controllers
{
    public class ParaleloController : Controller
    {
        ...
        public ActionResult Nuevo(aca_Paralelo_Info model)
        {
            ...
            return RedirectToAction("Consultar", new { IdEmpresa = model.IdEmpresa, IdParalelo = model.IdParalelo, Exito = true });
        }

        public ActionResult Consultar(int IdEmpresa = 0, int IdParalelo = 0, bool Exito = false)
        {
            #region Validar Session
            if (string.IsNullOrEmpty(SessionFixed.IdTransaccionSession))
                return RedirectToAction("Login", new { Area = "", Controller = "Account" });
