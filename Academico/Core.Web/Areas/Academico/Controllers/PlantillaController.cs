...
namespace Core.Web.Areas.Academico.Controllers
{
    public class PlantillaController : Controller
    {
        ...
        public ActionResult Nuevo(aca_Plantilla_Info model)
        {
            ...
            return RedirectToAction("Consultar", new { IdEmpresa = model.IdEmpresa, IdAnio = model.IdAnio, IdPlantilla = model.IdPlantilla, Exito = true });
        }

        public ActionResult Consultar(int IdEmpresa = 0, int IdAnio = 0, int IdPlantilla = 0, bool Exito = false)
        {
            #region Validar Session
            if (string.IsNullOrEmpty(SessionFixed.IdTransaccionSession))
                return RedirectToAction("Login", new { Area = "", Controller = "Account" });
