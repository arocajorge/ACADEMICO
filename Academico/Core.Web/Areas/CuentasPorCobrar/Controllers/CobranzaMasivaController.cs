            ...
            if (stream.Length > 0)
            {
                ...
                while (reader.Read())
                {
                    if (!reader.IsDBNull(0) && cont > 0)
                    {
                        ...
                        var ValorCxC = Math.Round((Convert.ToDouble(lst_AlumnoCartertaXCobrar == null ? 0 : lst_AlumnoCartertaXCobrar.Sum(q => q.ValorProntoPago - q.TotalxCobrado))),2, MidpointRounding.AwayFromZero);
