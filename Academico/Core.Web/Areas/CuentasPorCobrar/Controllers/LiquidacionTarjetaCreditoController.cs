...
namespace Core.Web.Areas.CuentasPorCobrar.Controllers
{
    public class LiquidacionTarjetaCreditoController : Controller
    {
        ...
        public ActionResult Nuevo(int IdEmpresa = 0)
        {
            ...
            #region Permisos
            aca_Menu_x_seg_usuario_Info info = bus_permisos.get_list_menu_accion(Convert.ToInt32(SessionFixed.IdEmpresa), Convert.ToInt32(SessionFixed.IdSede), SessionFixed.IdUsuario, "CuentasPorCobrar", "LiquidacionTarjetaCredito", "Index");
            if (!info.Nuevo)
        ...
        public ActionResult GridViewPartial_flujo_det()
        {
            SessionFixed.IdTransaccionSessionActual = Request.Params["TransaccionFixed"] != null ? Request.Params["TransaccionFixed"].ToString() : SessionFixed.IdTransaccionSessionActual;
            int IdEmpresa = Convert.ToInt32(SessionFixed.IdEmpresa);
            var model = List_LiquidacionTarjeta_Flujo.get_list(Convert.ToDecimal(SessionFixed.IdTransaccionSessionActual));
            return PartialView("_GridViewPartial_flujo_det", model);
        }
        [HttpPost, ValidateInput(false)]
        public ActionResult EditingAddNewFlujo([ModelBinder(typeof(DevExpressEditorsBinder))] cxc_LiquidacionTarjeta_x_ba_TipoFlujo_Info info_det)
        {
            if (ModelState.IsValid)
                List_LiquidacionTarjeta_Flujo.AddRow(info_det, Convert.ToDecimal(SessionFixed.IdTransaccionSessionActual));
            var model = List_LiquidacionTarjeta_Flujo.get_list(Convert.ToDecimal(SessionFixed.IdTransaccionSessionActual));
            return PartialView("_GridViewPartial_flujo_det", model);
        }
        [HttpPost, ValidateInput(false)]
        public ActionResult EditingUpdateFlujo([ModelBinder(typeof(DevExpressEditorsBinder))] cxc_LiquidacionTarjeta_x_ba_TipoFlujo_Info info_det)
        {
            if (ModelState.IsValid)
                List_LiquidacionTarjeta_Flujo.UpdateRow(info_det, Convert.ToDecimal(SessionFixed.IdTransaccionSessionActual));
            var model = List_LiquidacionTarjeta_Flujo.get_list(Convert.ToDecimal(SessionFixed.IdTransaccionSessionActual));
            return PartialView("_GridViewPartial_flujo_det", model);
        }
        public ActionResult EditingDeleteFlujo(int Secuencia)
        {
            List_LiquidacionTarjeta_Flujo.DeleteRow(Secuencia, Convert.ToDecimal(SessionFixed.IdTransaccionSessionActual));
            var model = List_LiquidacionTarjeta_Flujo.get_list(Convert.ToDecimal(SessionFixed.IdTransaccionSessionActual));
            return PartialView("_GridViewPartial_flujo_det", model);
        }
        #endregion
    ...
    public class cxc_LiquidacionTarjeta_x_ba_TipoFlujo_List
    {
        ...
        public List<cxc_LiquidacionTarjeta_x_ba_TipoFlujo_Info> get_list(decimal IdTransaccionSession)
        {
            if (HttpContext.Current.Session[Variable + IdTransaccionSession.ToString()] == null)
            {
                List<cxc_LiquidacionTarjeta_x_ba_TipoFlujo_Info> list = new List<cxc_LiquidacionTarjeta_x_ba_TipoFlujo_Info>();

                HttpContext.Current.Session[Variable + IdTransaccionSession.ToString()] = list;
            }
            return (List<cxc_LiquidacionTarjeta_x_ba_TipoFlujo_Info>)HttpContext.Current.Session[Variable + IdTransaccionSession.ToString()];
        }
        public void set_list(List<cxc_LiquidacionTarjeta_x_ba_TipoFlujo_Info> list, decimal IdTransaccionSession)
        {
            HttpContext.Current.Session[Variable + IdTransaccionSession.ToString()] = list;
        }
