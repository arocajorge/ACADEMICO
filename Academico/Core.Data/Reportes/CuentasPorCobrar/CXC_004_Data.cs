        ...
        public List<CXC_004_Info> Getlist_Saldo(int IdEmpresa, DateTime FechaCorte)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(CadenaDeConexion.GetConnectionString()))
                {
                    ...
                    + " select a.IdCtaCbleDebe, a.pc_Cuenta, sum(a.Saldo) Debe, 0 Haber "
                    + " from( "
                        + " select a.IdEmpresa, a.IdSucursal, a.IdBodega, a.IdCbteVta, a.vt_tipoDoc, a.vt_fecha, b.Total - isnull(f.dc_ValorPago, 0) Saldo, a.vt_Observacion, d.IdCtaCbleDebe, e.pc_Cuenta "
                        ...
                        + " ct_plancta as e on d.IdEmpresa = e.IdEmpresa and d.IdCtaCbleDebe = e.IdCtaCble left join "
                        + " ( "
                        ...
                        + " UNION ALL "
                        + " SELECT a.IdEmpresa, a.IdSucursal, a.IdBodega, a.IdNota, a.CodDocumentoTipo, a.no_fecha, b.Total - isnull(f.dc_ValorPago, 0) as Saldo, a.sc_observacion, a.IdCtaCble_TipoNota, d.pc_Cuenta "
                        ...
                        + " and dbo.BankersRounding(b.Total - isnull(f.dc_ValorPago, 0), 2) > 0 "
                    + " ) "
                    + " a group by a.IdCtaCbleDebe, a.pc_Cuenta "
                    + " UNION ALL "
                    + " SELECT a.IdCtaCble_TipoNota, d.pc_Cuenta, 0 as Debe, dbo.BankersRounding(sum(b.Total) - sum(ISNULL(C.Valor_Aplicado, 0)), 2) Haber "
                        ...
                        + " ct_plancta as d on a.IdEmpresa = d.IdEmpresa and a.IdCtaCble_TipoNota = d.IdCtaCble "
                        + " WHERE A.IdEmpresa = @IdEmpresa AND A.no_fecha <= @FechaFin AND A.Estado = 'A' and a.CreDeb = 'C' "
                        + " group by a.IdCtaCble_TipoNota, d.pc_Cuenta "
