        ...
        public List<CXC_004_Info> Getlist_Saldo(int IdEmpresa, DateTime FechaCorte)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(CadenaDeConexion.GetConnectionString()))
                {
                    ...
                    + " from( "
                        + " select a.IdEmpresa, a.IdSucursal, a.IdBodega, a.IdCbteVta, a.vt_tipoDoc, a.vt_fecha, b.Total - isnull(f.dc_ValorPago, 0) Saldo, a.vt_Observacion, case when g.EnCurso = 1 then d.IdCtaCbleDebe else g.IdCtaCbleCierre end IdCtaCbleDebe, e.pc_Cuenta "
