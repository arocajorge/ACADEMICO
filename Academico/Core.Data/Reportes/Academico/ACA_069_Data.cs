    ...
    public class ACA_069_Data
    {
        ...
        public List<ACA_069_Info> GetList(int IdEmpresa, int IdAnio, int IdSede, int IdNivel, int IdJornada, int IdCurso, int IdParalelo, bool MostrarRetirados)
        {
            try
            {
                ...
                List<ACA_069_Info> ListaFinal = new List<ACA_069_Info>();
                ...
                var lst_promedio_complementarias_PromFinal = new List<ACA_069_Info>();
                foreach (var item in ListaComplementariasProm_Final)
                {
                    lst_promedio_complementarias_PromFinal.Add(new ACA_069_Info
                    {
                ...
                var lst_promedio_general = new List<ACA_069_Info>();
                foreach (var item in ListaPromedioGeneral)
                {
                    lst_promedio_general.Add(new ACA_069_Info
                    {
                        IdEmpresa = item.IdEmpresa,
                        IdMatricula = item.IdMatricula,
                        IdMateria = 0,
                ...
                var ListaAlumnos = lst_promedio_general.Where(q => q.CalificacionNumerica != null && q.CalificacionNumerica >= Convert.ToDecimal(9.00) && q.CalificacionNumerica <= Convert.ToDecimal(9.50)).ToList();
                return ListaAlumnos;
            }
            catch (Exception ex)
            {
