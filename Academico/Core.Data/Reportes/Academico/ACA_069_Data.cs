    ...
    public class ACA_069_Data
    {
        ...
        public List<ACA_069_Info> GetList(int IdEmpresa, int IdAnio, int IdSede, int IdNivel, int IdJornada, int IdCurso, int IdParalelo, bool MostrarRetirados)
        {
            try
            {
                ...
                var ListaPromediar = new List<ACA_069_Info>();
                ListaPromediar.AddRange(lstPromedioObligatorias);
                ListaPromediar.AddRange(lstPromedioComplementarias);

                var lstFinal = ListaPromediar.GroupBy(q => new { q.IdEmpresa, q.IdMatricula }).Select(q => new ACA_069_Info
                {
                    IdEmpresa = q.Key.IdEmpresa,
                    IdMatricula = q.Key.IdMatricula,
                ...
                var ListaFinal = ListaPromediar.Where(q => q.PromedioCalculado < Convert.ToDecimal(9.50) && q.PromedioCalculado >= Convert.ToDecimal(9.00)).ToList();
                return ListaFinal;
            }
            catch (Exception ex)
            {
