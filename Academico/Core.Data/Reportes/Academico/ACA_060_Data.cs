        ...
        public List<ACA_060_Info> GetList(int IdEmpresa, int IdAnio, int IdSede, int IdNivel, int IdJornada, int IdCurso, int IdParalelo)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(CadenaDeConexion.GetConnectionString()))
                {
                    ...
                    + " dbo.aca_Matricula AS m ON m.IdEmpresa = mp.IdEmpresa AND m.IdMatricula = mp.IdMatricula LEFT OUTER JOIN "
                    + " dbo.aca_AnioLectivo_Curso_Paralelo AS cp ON m.IdAnio = cp.IdAnio AND m.IdSede = cp.IdSede AND m.IdNivel = cp.IdNivel AND m.IdJornada = cp.IdJornada AND m.IdCurso = cp.IdCurso AND "
                    + " m.IdParalelo = cp.IdParalelo "
                    + " LEFT OUTER JOIN "
                    + " dbo.aca_AnioLectivo_Tematica AS t ON mp.IdEmpresa = t.IdEmpresa AND mp.IdCampoAccion = t.IdCampoAccion AND mp.IdTematica = t.IdTematica LEFT OUTER JOIN "
                    ...
                    + " LEFT OUTER JOIN "
                    + " dbo.aca_Profesor AS p ON mp.IdEmpresa = p.IdEmpresa AND mp.IdProfesor = p.IdProfesor LEFT OUTER JOIN "
                    + " dbo.tb_persona AS pe ON pe.IdPersona = p.IdPersona "
