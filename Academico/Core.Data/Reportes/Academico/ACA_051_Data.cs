        ...
        public List<ACA_051_Info> GetList(int IdEmpresa, int IdAnio, int IdSede, int IdNivel, int IdJornada, int IdCurso,int IdParalelo)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(CadenaDeConexion.GetConnectionString()))
                {
                    ...
                    + " FROM dbo.aca_AlumnoRetiro AS r WITH (nolock) INNER JOIN "
                    + " dbo.aca_Matricula AS m WITH (nolock) ON r.IdEmpresa = m.IdEmpresa AND r.IdMatricula = m.IdMatricula INNER JOIN "
                    ...
                    + " dbo.aca_Alumno AS al WITH (nolock) INNER JOIN "
                    + " dbo.tb_persona AS p WITH (nolock) ON al.IdPersona = p.IdPersona ON m.IdEmpresa = al.IdEmpresa AND m.IdAlumno = al.IdAlumno "
