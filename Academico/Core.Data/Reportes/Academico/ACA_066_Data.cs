        ...
        public List<ACA_066_Info> GetList(int IdEmpresa, int IdAnio)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(CadenaDeConexion.GetConnectionString()))
                {
                    ...
                    + " aca_Matricula as d on a.IdEmpresa = d.IdEmpresa and a.IdAlumno = d.IdAlumno left join "
                    + " aca_AnioLectivo_NivelAcademico_Jornada as nj on d.IdEmpresa = nj.IdEmpresa and d.IdAnio = nj.IdAnio and d.IdSede = nj.IdSede and d.IdJornada = nj.IdJornada and d.IdNivel = nj.IdNivel left join "
                    + " aca_AnioLectivo_Jornada_Curso as jc on jc.IdEmpresa = d.IdEmpresa and jc.IdAnio = d.IdAnio and jc.IdSede = d.IdSede and jc.IdJornada = d.IdJornada and jc.IdNivel = d.IdNivel and jc.IdCurso = d.IdCurso left join "
                    + " aca_AnioLectivo_Curso_Paralelo as cp on cp.IdEmpresa = d.IdEmpresa and cp.IdAnio = d.IdAnio and cp.IdSede = d.IdSede and cp.IdJornada = d.IdJornada and cp.IdNivel = d.IdNivel and cp.IdCurso = d.IdCurso and cp.IdParalelo = d.IdParalelo left join "
                    + " aca_Plantilla as p on d.IdEmpresa = p.IdEmpresa and d.IdAnio = p.IdAnio and d.IdPlantilla = p.IdPlantilla left join "
                    + " aca_PlantillaTipo as pt on pt.IdTipoPlantilla = p.IdTipoPlantilla and pt.IdEmpresa = p.IdEmpresa left join "
                    + " aca_AnioLectivo as al on d.IdEmpresa = al.IdEmpresa and d.IdAnio = al.IdAnio "
                    + " where a.IdEmpresa = @IdEmpresa and d.IdAnio = @IdAnio and c.IdCatalogoPAREN in (10,11) and exists("
                        + " select f.IdEmpresa "
                        + " from aca_Familia as f "
                        + " join "
                    + " aca_Alumno as f1 on f.IdEmpresa = f1.IdEmpresa and f.IdAlumno = f1.IdAlumno join "
                        + " aca_Matricula as f2 on f2.IdEmpresa = f1.IdEmpresa and f2.IdAlumno = f1.IdAlumno "
