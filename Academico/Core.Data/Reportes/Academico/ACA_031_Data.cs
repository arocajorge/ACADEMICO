...
{
    public class ACA_031_Data
    {
        public List<ACA_031_Info> get_list(int IdEmpresa, int IdAnio, int IdSede, int IdNivel, int IdJornada, int IdCurso, int IdParalelo, string IdCatalogoParcialTipo)
        {
            try
            {
                List<ACA_031_Info> Lista = new List<ACA_031_Info>();
                using (EntitiesReportes Context = new EntitiesReportes())
                {
                    Context.Database.CommandTimeout = 5000;
                    var lst = Context.SPACA_031(IdEmpresa, IdAnio, IdSede, IdNivel, IdJornada, IdCurso, IdParalelo).ToList();

                    foreach (var q in lst)
                    {
                        Lista.Add(new ACA_031_Info
                        {
                            Num = 1,
