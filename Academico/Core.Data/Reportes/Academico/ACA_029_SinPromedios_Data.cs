...
{
    public class ACA_029_SinPromedios_Data
    {
        public List<ACA_029_SinPromedios_Info> get_list(int IdEmpresa, int IdAnio, int IdSede, int IdNivel, int IdJornada, int IdCurso, int IdParalelo, int IdMateria, int IdCatalogoTipo)
        {
            try
            {
                List<ACA_029_SinPromedios_Info> Lista = new List<ACA_029_SinPromedios_Info>();
                List<ACA_029_SinPromedios_Info> ListaFinal = new List<ACA_029_SinPromedios_Info>();
                using (EntitiesReportes Context = new EntitiesReportes())
                {
                    Context.Database.CommandTimeout = 5000;
                    var lst = Context.SPACA_029_SinPromedios(IdEmpresa, IdAnio, IdSede, IdNivel, IdJornada, IdCurso, IdParalelo, IdMateria).ToList();

                    foreach (var q in lst)
                    {
                        Lista.Add(new ACA_029_SinPromedios_Info
                        {
