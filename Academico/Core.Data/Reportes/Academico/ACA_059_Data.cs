    ...
    public class ACA_059_Data
    {
        public List<ACA_059_Info> GetList(int IdEmpresa, int IdAnio, int IdSede, int IdNivel, int IdJornada, int IdCurso, int IdParalelo, bool MostrarRetirados)
        {
            try
            {
                ...
                List<ACA_059_Info> Lista = new List<ACA_059_Info>();
                using (SqlConnection connection = new SqlConnection(CadenaDeConexion.GetConnectionString()))
                {
                    ...
                    + " SELECT ROW_NUMBER() OVER (PARTITION BY m.IdEmpresa, m.IdAnio, m.IdSede, m.IdNivel, m.IdJornada "
                    + " ORDER BY nj.OrdenJornada,sn.OrdenNivel, jc.OrdenCurso, p.pe_nombreCompleto ASC) Num, m.IdEmpresa, m.IdMatricula, m.IdAnio, m.IdSede, m.IdNivel, m.IdJornada, m.IdCurso, m.IdParalelo, m.IdAlumno, CASE WHEN r.IdRetiro IS NULL THEN CAST(0 AS BIT) ELSE CAST(1 AS BIT) END AS EsRetirado, an.Descripcion, sn.NomSede, sn.NomNivel, "
                    ...
                    + " ( "
                    + " SELECT ROW_NUMBER() OVER(PARTITION BY m.IdEmpresa, m.IdAnio, m.IdSede, m.IdNivel, m.IdJornada "
                    + " ORDER BY nj.OrdenJornada, sn.OrdenNivel, jc.OrdenCurso, p.pe_nombreCompleto ASC), m.IdEmpresa, m.IdMatricula, m.IdAnio, m.IdSede, 2, m.IdJornada, m.IdCurso, m.IdParalelo, m.IdAlumno, CASE WHEN r.IdRetiro IS NULL THEN CAST(0 AS BIT) ELSE CAST(1 AS BIT) END AS EsRetirado, an.Descripcion, sn.NomSede, sn.NomNivel, "
                    + " sn.OrdenNivel, nj.NomJornada, nj.OrdenJornada, jc.NomCurso, jc.OrdenCurso, cp.NomParalelo, cp.OrdenParalelo,p.pe_nombreCompleto "
                    + " FROM     dbo.aca_Matricula AS m WITH (nolock) LEFT OUTER JOIN "
                    + " dbo.aca_Alumno AS al WITH (nolock) ON m.IdEmpresa = al.IdEmpresa AND m.IdAlumno = al.IdAlumno LEFT OUTER JOIN "
                    + " dbo.tb_persona AS p WITH (nolock) ON al.IdPersona = p.IdPersona LEFT OUTER JOIN "
