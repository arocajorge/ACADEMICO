        ...
        public List<ACA_067_Info> GetList(int IdEmpresa, int IdAnio, int IdSede, int IdNivel, int IdJornada, int IdCurso, int IdParalelo, bool MostrarRetirados)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(CadenaDeConexion.GetConnectionString()))
                {
                    ...
                    + " dbo.aca_MatriculaCondicional AS mc WITH (nolock) ON m.IdEmpresa = mc.IdEmpresa AND m.IdAlumno = mc.IdAlumno AND m.IdAnio = mc.IdAnio and mc.Estado=1 LEFT OUTER JOIN "
                    + " dbo.tb_persona AS rep WITH (nolock) ON rep.IdPersona = m.IdPersonaR LEFT OUTER JOIN "
