        ...
        public List<cxc_cobro_Info> get_list(int IdEmpresa, int IdSucursal, DateTime Fecha_ini, DateTime Fecha_fin)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(CadenaDeConexion.GetConnectionString()))
                {
                    ...
                    #region Query
                    string query = "SELECT FC.IdPersona, C.IdEmpresa, C.IdSucursal, C.IdCobro, C.IdCobro_tipo, C.IdCliente, C.cr_TotalCobro, C.cr_fecha, C.cr_fechaCobro, C.cr_fechaDocu, C.cr_observacion, C.cr_Banco, C.cr_cuenta, C.cr_Tarjeta, C.cr_NumDocumento, "
                    ...
                    + " dbo.tb_sucursal AS S WITH(nolock) INNER JOIN "
                    + " dbo.cxc_cobro AS C WITH(nolock) ON S.IdEmpresa = C.IdEmpresa AND S.IdSucursal = C.IdSucursal INNER JOIN "
                    + " dbo.fa_cliente AS FC WITH(nolock) ON C.IdCliente = FC.IdCliente AND C.IdEmpresa = FC.IdEmpresa ON P.IdPersona = FC.IdPersona "
                    + " WHERE C.IdEmpresa = " + IdEmpresa.ToString() + "and C.IdSucursal between " + IdSucursal_ini.ToString() + " and " + IdSucursal_fin.ToString()
                    + " and ";
                    #endregion
                    ...
                    while (reader.Read())
                    {
                        Lista.Add(new cxc_cobro_Info
                        {
                            ...
                            IdCobro_tipo = string.IsNullOrEmpty(reader["IdCobro_tipo"].ToString()) ? null : reader["IdCobro_tipo"].ToString(),
                            cr_fecha = Convert.ToDateTime(reader["cr_fecha"]),
                            cr_TotalCobro = Convert.ToDouble(reader["cr_TotalCobro"]),
                            cr_estado = string.IsNullOrEmpty(reader["cr_estado"].ToString()) ? null : reader["cr_estado"].ToString(),
                            Su_Descripcion = string.IsNullOrEmpty(reader["Su_Descripcion"].ToString()) ? null : reader["Su_Descripcion"].ToString(),
                            cr_observacion = string.IsNullOrEmpty(reader["cr_observacion"].ToString()) ? null : reader["cr_observacion"].ToString(),
                            nom_Motivo_tipo_cobro = string.IsNullOrEmpty(reader["nom_Motivo_tipo_cobro"].ToString()) ? null : reader["nom_Motivo_tipo_cobro"].ToString(),
                            cr_NumDocumento = string.IsNullOrEmpty(reader["cr_NumDocumento"].ToString()) ? null : reader["cr_NumDocumento"].ToString(),
                            Usuario = string.IsNullOrEmpty(reader["IdUsuario"].ToString()) ? null : reader["IdUsuario"].ToString(),
                            EstadoBool = string.IsNullOrEmpty(reader["cr_estado"].ToString()) ? false : (reader["cr_estado"].ToString()=="A" ? true : false)
                        });
                    }
                    reader.Close();
                }
        ...
        public List<cxc_cobro_Info> get_list_matricula(int IdEmpresa, int IdSucursal, DateTime Fecha_ini, DateTime Fecha_fin)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(CadenaDeConexion.GetConnectionString()))
                {
                    ...
                    + " dbo.cxc_cobro WITH (nolock)INNER JOIN "
                    + " dbo.fa_cliente WITH (nolock)ON dbo.cxc_cobro.IdEmpresa = dbo.fa_cliente.IdEmpresa AND dbo.cxc_cobro.IdCliente = dbo.fa_cliente.IdCliente INNER JOIN "
                    + " dbo.tb_persona WITH (nolock)ON dbo.fa_cliente.IdPersona = dbo.tb_persona.IdPersona INNER JOIN "
                    + " dbo.tb_sucursal WITH (nolock)ON dbo.cxc_cobro.IdEmpresa = dbo.tb_sucursal.IdEmpresa AND dbo.cxc_cobro.IdSucursal = dbo.tb_sucursal.IdSucursal ON dbo.aca_Alumno.IdEmpresa = dbo.cxc_cobro.IdEmpresa AND "
                    + " dbo.aca_Alumno.IdAlumno = dbo.cxc_cobro.IdAlumno LEFT OUTER JOIN "
                      + " (SELECT dbo.cxc_cobro_det.IdEmpresa, dbo.cxc_cobro_det.IdSucursal, dbo.cxc_cobro_det.IdCobro, dbo.cxc_cobro_tipo_motivo.nom_Motivo_tipo_cobro "
                       + " FROM dbo.cxc_cobro_det WITH (nolock)INNER JOIN "
                       + " dbo.cxc_cobro_tipo WITH (nolock)ON dbo.cxc_cobro_det.IdCobro_tipo = dbo.cxc_cobro_tipo.IdCobro_tipo INNER JOIN "
                       + " dbo.cxc_cobro_tipo_motivo WITH (nolock)ON dbo.cxc_cobro_tipo.IdMotivo_tipo_cobro = dbo.cxc_cobro_tipo_motivo.IdMotivo_tipo_cobro "
                    ...
                    + " dbo.cxc_cobro_tipo AS t1 WITH (nolock)ON dbo.cxc_cobro.IdCobro_tipo = t1.IdCobro_tipo LEFT OUTER JOIN "
                    + " dbo.cxc_cobro_tipo_motivo AS t2 WITH (nolock)ON t1.IdMotivo_tipo_cobro = t2.IdMotivo_tipo_cobro LEFT OUTER JOIN "
                    + " dbo.ba_Banco_Cuenta AS ba WITH (nolock)ON dbo.cxc_cobro.IdEmpresa = ba.IdEmpresa AND dbo.cxc_cobro.IdBanco = ba.IdBanco "
