    ...
    public class ba_ArchivoRecaudacion_Data
    {
        public List<ba_ArchivoRecaudacion_Info> GetList(int IdEmpresa, DateTime fechaini, DateTime fechafin, bool mostrar_anulados)
        {
            try
            {
                ...
                {
                    var lst = Context.ba_ArchivoRecaudacion.Where(q => q.IdEmpresa == IdEmpresa && q.Fecha >= fechaini.Date && q.Fecha <= fechafin.Date && q.Estado == (mostrar_anulados == true ? q.Estado : true)).ToList();
